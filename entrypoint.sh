...
popd >/dev/null || error_exit "could not popd"
#log_info "[whamm] Building cache."
#pushd ./user_libs/cache >/dev/null || error_exit "could not pushd"
#rustup target add wasm32-wasip1
#cargo build --release --target wasm32-wasip1
#popd >/dev/null || error_exit "could not popd"
...
# TODO:
# - pull result csv file name from $run_out
# - pass that to the plotting scripts (shouldn't need to do post-processing, just use the whole file)
# - rework the plotting scripts to be standalone python files
