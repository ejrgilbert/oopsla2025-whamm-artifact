...
> Do the following steps to run the artifact on an `amd64` machine:
...
> - Skip generating a plot for the experiment: Comment out the `if` statement that executes the `whamm-vs-pin.py` script at the bottom of the `./entrypoint.sh` script.
...
It is recommended to do the following to validate the setup of the artifact (should complete in under 30 minutes depending on your system). Doing so will result  in running end-to-end on a single benchmark by running each experiment configuration once and timing out if a configuration runs for over 5 seconds.
