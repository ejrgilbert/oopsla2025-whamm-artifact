...
#  - pin data is in the same file as all other data
#  - pin results have the same timeout
#  - pin data is cleaned correctly by utils::clean function
#  - pintools share the same naming conventions as other framework monitors
#  - pin results can be pulled via 'run_time:mean'
#  - pine baselines can be pulled via:
#    - pin running with no tool activated:
#           'config:run_mode' == 'base_pin' && 'config:special' == 'no-tool'
#    - pin running with an empty tool of the specific monitor:
#           'config:run_mode' == 'base_pin' && 'config:special' == 'empty-tool' && 'config:monitor' == MONITOR_NAME
#    - pin running with the actual pintool of a specific monitor:
#           'config:run_mode' == 'pin' && 'config:monitor' == MONITOR_NAME
...
def whamm_engine_df(mon):
    ...
    for benchmark_name in monitor_polybench_df['benchmark:name'].unique():
        ...
        mean = \
        engine_data[(engine_data['config:run_mode'] == MODE) & (engine_data['config:special'].isna())][MEAN].values[0]
        mean_bundle = \
        engine_data[(engine_data['config:run_mode'] == MODE) & (engine_data['config:special'] == SPEC_BUNDLE)][
            MEAN].astype(float).values[0]

        bind_time_us = \
        engine_data[(engine_data['config:run_mode'] == MODE) & (engine_data['config:special'].isna())][WIZ_BIND].astype(
            float).values[0]
...
# Pin data
SPEC_NOTOOL = 'no-tool'
SPEC_EMPTY_TOOL = 'empty-tool'

polybench_pin_df = df[(df['config:run_mode'] == 'pin') & (df['benchmark:suite'] == SUITE)]
baseline_polybench_mach_df = df[(df['config:run_mode'] == 'base_mach') & (df['benchmark:suite'] == SUITE)]
baseline_polybench_pin_df = df[(df['config:run_mode'] == 'base_pin') & (df['benchmark:suite'] == SUITE)]

def pin_df(mon):
    # Monitor data, polybench data!
    monitor_polybench_df = polybench_pin_df[polybench_pin_df['config:monitor'] == mon]

    new_df_rows = []
    for benchmark_name in monitor_polybench_df['benchmark:name'].unique():
        baseline_pin_data = baseline_polybench_pin_df[baseline_polybench_pin_df['benchmark:name'] == benchmark_name]

        base_uninstr = baseline_polybench_mach_df[baseline_polybench_mach_df['benchmark:name'] == benchmark_name][MEAN].values[0]
        base_notool = baseline_pin_data[baseline_pin_data['config:special'] == SPEC_NOTOOL][MEAN].values[0]
        base_instr_empty = baseline_pin_data[(baseline_pin_data['config:monitor'] == mon) & (baseline_pin_data['config:special'] == SPEC_EMPTY_TOOL)][MEAN].values[0]

        # Includes bind/instr/report times
        pin_instr_full = monitor_polybench_df[monitor_polybench_df['benchmark:name'] == benchmark_name][MEAN].values[0]
